...
test.describe('Mind Wallet 2.0 smoke', () => {
  test('login, dashboard, extrato, favorecido, PIX, notificação e onboarding', async ({
    page,
  }) => {
    await login(page)
    ...
    await expect(page.getByTestId('onboarding-step-PROFILE_OK')).toHaveClass(
      /done/,
    )
    ...
    await expect(
      page.getByTestId('onboarding-step-VIEW_STATEMENT'),
    ).toHaveClass(/done/)
    ...
    await expect(page.getByTestId('beneficiary-list')).toContainText(
      'E2E Favorecido',
    )

    await nav(page).getByRole('link', { name: 'Início' }).click()
    await expect(
      page.getByTestId('onboarding-step-FIRST_BENEFICIARY'),
    ).toHaveClass(/done/)
    ...
    await nav(page).getByRole('link', { name: 'Início' }).click()
    await expect(page.getByTestId('onboarding-step-FIRST_PIX')).toHaveClass(
      /done/,
    )
